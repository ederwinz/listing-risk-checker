...
# Review failed match candidates and add them as aliases (run after --all or --watch)
python scrapers/update_aliases.py
# Interactive: shows each PRODUCT_LINE_NOT_FOUND / COLORWAY_NOT_FOUND case,
# lets you map it to an official name, and writes both aliases.json files automatically.
...
- `aliases.json` (at `Data-Scrapers/aliases.json` and a copy at `frontend/data/aliases.json`) — lookup table bridging Chinese marketplace terms to official English product names in `verified_products`. Two roles: **product-line aliases** (e.g. `"双饮杯"` → `"FreeSip"`, checked after exact+fuzzy matching fails) and **colorway color tags** (e.g. `"light pink"` → `"Bunny Business"`, checked when extraction falls back to visual color via the `main_colors` field). Structure: `brand → product_line → { aliases: [...], colorways: { official_name: [color tags] } }`. `Data-Scrapers/aliases.json` is authoritative — copy to `frontend/data/aliases.json` before deploying. `update_aliases.py` writes both files automatically.
- `scrapers/update_aliases.py` — reads `alias_candidates.jsonl` (auto-written by `comparison_engine.py` on each failed match), uses Claude to map each unrecognized term to the correct official product line or colorway, and appends confirmed mappings to both `aliases.json` files. Run after any batch comparison to expand alias coverage.
...
Loads `data/aliases.json` at import time (JSON import, no runtime fetch) for alias-based product-line matching and visual color → colorway mapping. If you update `Data-Scrapers/aliases.json`, copy it to `frontend/data/aliases.json` before deploying.
...
All shared TypeScript types are in `types/report.ts`: `Report`, `RiskLevel`, `MatchType`, `Extracted`, `VerifiedProduct`, `ResultSlot`. `Extracted` includes `main_colors` (visual color descriptions from product photos, used for color-tag colorway matching). `MatchContext` has an optional `color_tag_match: boolean` flag set when a match was found via color tags rather than name similarity.
